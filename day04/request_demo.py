#导入 request第三方包(第三方类库)
import requests

if __name__ == '__main__':
    #声明login_date 变量名 用来存 登录的请求数据
    login_date = {"username":"admin","password":"123456"}
    # = 后面是发起一个请求 使用requests.post方法，入参 有两个 ，url：请求地址， json：请求数据
    # = 前面 就是声明一个变量， 来存方法执行完成后 return 的内容
    login_resp = requests.post(url='http://192.168.60.132:8080/admin/login', json=login_date)
    print(type(login_resp))
    # .text就是获取响应正文（str类型）
    login_resp_text = login_resp.text
    print(login_resp_text)

    #获取 响应正文（字典类型）  响应正文必须是json格式
    login_resp_json = login_resp.json()
    #取出响应正文中 key 为  message的值
